# 트리 시각화 도구 (Tree Helper)

데이터 구조 학습을 위한 인터랙티브 트리 시각화 웹 애플리케이션입니다. BST, AVL 트리, B-트리, B+ 트리의 삽입/삭제 연산을 단계별로 시각화하여 보여줍니다.

**📱 모바일 지원** - 터치 드래그, 반응형 UI, Safe Area 지원으로 모바일에서도 편리하게 사용 가능합니다.

## 주요 기능

### 1. 트리 시뮬레이션 모드

- **4가지 트리 타입 지원**

  - 이진 탐색 트리 (BST)
  - AVL 트리 (자가 균형 이진 탐색 트리)
  - B-트리
  - B+ 트리

- **단계별 시각화**

  - 각 연산의 모든 단계를 시각화로 표시
  - 노드 생성, 삽입, 삭제, 회전, 분할, 병합 등 세부 과정 확인 가능
  - 각 단계마다 한국어 설명 제공
  - B-트리/B+트리: 언더플로우 처리(재분배/병합) 과정 상세히 표시

- **명령어 입력 방식**

  - 개별 입력: 삽입/삭제 선택 후 값 입력
  - 일괄 입력: `i 30, d 45, i 20` 형식으로 여러 명령 한번에 입력
    - `i`: 삽입 (insert)
    - `d`: 삭제 (delete)
    - `i`, `d` 없을시: 삽입 (insert)
      - ex : 1, 2, 3, 4 = i 1, i 2, i 3, i 4

- **B-트리 차수 설정**

  - B-트리와 B+ 트리의 차수(m) 조정 가능
  - 기본값: 3, 범위: 3-10

- **시각화 컨트롤**
  - 이전/다음 단계 이동
  - 확대/축소 기능
  - 마지막 단계로 건너뛰기
  - 리셋 기능
  - 터치 드래그로 캔버스 이동 (모바일 지원)

### 2. 문제 풀이 모드

- **랜덤 문제 생성**

  - 연산 개수 설정 (5-30개)
  - 트리 타입 선택
  - B-트리 차수 설정

- **답안 형식**

  - **BST/AVL 트리**: `높이,루트키,왼쪽노드수,오른쪽노드수#단말노드1,단말노드2,...`
    - 예: `3,50,2,3#10,25,60,80`
  - **B-트리/B+ 트리**: `{키1,키2},{키3,키4,키5},...`
    - 예: `{10,20},{30,40,50}`

- **답안 확인 및 해설**
  - 정답/오답 즉시 확인
  - 정답 보기 버튼으로 시뮬레이션 자동 실행

## 기술 스택

- **프레임워크**: Next.js 16 (App Router)
- **언어**: TypeScript (strict mode, no any types)
- **스타일링**: Tailwind CSS 4
- **렌더링**: HTML5 Canvas API
- **상태 관리**: React Hooks (useState, useEffect, useCallback, useMemo)

## 프로젝트 구조

```
tree-helper/
├── app/
│   ├── page.tsx              # 메인 페이지 (Suspense로 HomeView 감싸기)
│   └── practice/
│       └── page.tsx          # 문제 풀이 페이지 (PracticeView 불러오기)
├── views/
│   ├── Home/
│   │   └── index.tsx         # 메인 페이지 View (트리 시뮬레이션 로직)
│   └── Practice/
│       └── index.tsx         # 문제 풀이 View (문제 생성 및 답안 체크 로직)
├── components/
│   ├── InputPanel.tsx        # 명령어 입력 패널
│   └── TreeVisualization.tsx # 트리 시각화 캔버스
├── lib/
│   ├── types.ts             # 타입 정의
│   ├── constants.ts         # 상수 정의
│   ├── bst.ts              # BST 구현
│   ├── avl.ts              # AVL 트리 구현
│   ├── btree.ts            # B-트리 구현
│   ├── bplustree.ts        # B+ 트리 구현
│   ├── treeLayout.ts       # 트리 레이아웃 알고리즘
│   ├── problemGenerator.ts  # 문제 생성 로직
│   └── renderers/
│       ├── BSTRenderer.ts   # BST/AVL 렌더러
│       ├── BTreeRenderer.ts # B-트리 렌더러
│       └── BPlusTreeRenderer.ts # B+ 트리 렌더러
└── README.md
```

## 설치 및 실행

### 요구 사항

- Node.js 18 이상

### 설치

```bash
npm install
```

### 개발 서버 실행

```bash
npm run dev
```

브라우저에서 [http://localhost:3000](http://localhost:3000)을 열어 확인하세요.

### 프로덕션 빌드

```bash
npm run build
npm start
```

### 타입 체크

```bash
npx tsc --noEmit
```

## 사용 방법

### 시뮬레이션 모드

1. 왼쪽 패널에서 트리 타입 선택
2. B-트리/B+ 트리의 경우 차수(m) 설정
3. 명령어 입력:
   - **개별 입력**: 삽입/삭제 선택 → 값 입력 → 추가 버튼
   - **일괄 입력**: `i 30, d 45, i 20` 형식으로 입력
4. "시작하기" 버튼 클릭
5. 이전/다음 버튼으로 각 단계 확인

### 문제 풀이 모드

1. 메인 페이지에서 "📝 문제 풀이 모드" 버튼 클릭
2. 트리 타입, 연산 개수 설정
3. "새 문제 생성" 버튼 클릭
4. 연산 목록을 보고 최종 트리 상태 계산
5. 답안 입력 후 "정답 확인" 버튼 클릭
6. "시뮬레이션" 버튼으로 해설 확인

## 주요 알고리즘

### 트리 레이아웃

- **BST/AVL**: 중위 순회(inorder traversal)를 사용하여 왼쪽에서 오른쪽 순서로 노드 배치
- **B-트리/B+ 트리**: 레벨별 BFS 방식으로 레이아웃 계산

### BST/AVL 삭제 알고리즘

**후계자 선택 전략** (균형 유지 최적화):
1. 왼쪽/오른쪽 서브트리의 높이 비교 → 높은 쪽 선택
2. 높이가 같으면 노드 개수 비교 → 많은 쪽 선택
3. 모두 같으면 왼쪽 우선

이를 통해 한쪽으로 치우친 삭제를 방지하고 트리의 균형을 유지합니다.

### B-트리/B+ 트리 삭제 알고리즘

**표준 알고리즘 구현** (삭제 → 언더플로우 체크 → 수정):

1. **삭제 수행**
   - 리프 노드: 키 직접 삭제
   - 내부 노드: 후계자(오른쪽 서브트리 최소값)로 대체 후 리프에서 삭제

2. **언더플로우 체크**
   - B-트리: `minKeys = ⌈m/2⌉ - 1`
   - B+트리: 리프 `⌈m/2⌉`, 내부 `⌈m/2⌉ - 1`
   - 최소 키 개수보다 적으면 수정 필요

3. **재분배 (Redistribution)**
   - Best Sibling 선택: 키가 많은 형제 우선, 같으면 왼쪽 우선
   - 형제에서 키 차출 가능하면 재분배 수행
   - **B+트리**: 부모 키는 항상 왼쪽 자식의 최대값으로 업데이트 (≤ 라우팅 조건)

4. **병합 (Merge)**
   - 재분배 불가능하면 왼쪽 형제와 병합
   - 부모 키를 가져와서 두 노드 합치기
   - 부모 노드에서 언더플로우 재귀 전파

5. **루트 처리**
   - 루트가 비면 자식을 새 루트로 승격 (트리 높이 감소)

### 연산 단계 추적

각 트리 클래스는 삽입/삭제 연산 시 모든 중간 상태를 `OperationStep` 배열로 반환:

- 노드 생성
- 회전 (AVL)
- 분할/병합/재분배 (B-트리) - 상세한 설명과 함께
- 최종 상태

### 렌더링 분리

View 로직을 별도 Renderer 클래스로 분리하여 관심사 분리 및 재사용성 향상

## 타입 안전성

- 모든 코드는 TypeScript strict mode로 작성
- `any` 타입 사용 금지
- 명시적 타입 선언으로 런타임 에러 방지

## 기여

이슈 및 풀 리퀘스트를 환영합니다.

## 개발 노트

### 주요 설계 결정

1. **Views 아키텍처**: 페이지별 폴더 구조로 View 로직 분리

   - `app/` - 라우팅과 Suspense 경계 관리만 담당
   - `views/` - 페이지별 비즈니스 로직과 상태 관리
   - 관심사 분리로 유지보수성 및 테스트 용이성 향상

2. **Canvas API 사용**: SVG 대비 대량의 노드 렌더링 시 성능 우수

3. **Renderer 패턴**: 트리 로직과 렌더링 로직 분리로 유지보수성 향상

   - 각 트리 타입별 독립적인 Renderer 클래스
   - 트리 데이터 구조와 시각화 로직의 완전한 분리

4. **Step-by-step 추적**: 각 연산의 모든 중간 상태를 저장하여 교육 효과 극대화

5. **Inorder 레이아웃**: 노드 겹침 방지를 위한 중위 순회 기반 레이아웃 (BST/AVL)

6. **정확한 알고리즘 구현**:
   - B-tree/B+tree 삭제: 표준 알고리즘 (삭제 후 언더플로우 체크)
   - 언더플로우 재귀 전파로 모든 노드 조건 유지
   - Best Sibling 선택으로 최적의 재분배

### 최근 주요 업데이트

- **B-트리/B+트리 삭제 알고리즘 완전 재작성** (2025-12-08)
  - 언더플로우 체크 및 재귀 전파 구현
  - 재분배/병합 로직 정확히 작동
  - B+트리 라우팅 조건(≤)에 맞는 부모 키 업데이트
  - 병합 시각화 개선 (중복 제거, 명확한 설명)

- **BST/AVL 삭제 로직 개선**
  - 서브트리 높이/노드 개수 기반 후계자 선택
  - 균형 유지 최적화

- **모바일 최적화**
  - 터치 드래그 지원
  - 반응형 레이아웃
  - Safe Area 지원

### 향후 개선 사항

- [ ] 애니메이션 효과 추가
- [ ] 더 많은 트리 타입 지원 (Red-Black Tree, Splay Tree 등)
- [ ] 문제 풀이 기록 저장
- [ ] 트리 상태 공유 기능 개선 (현재 기본 지원됨)
